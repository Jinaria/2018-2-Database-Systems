...
public:
	...
	lock_t(int table_id, pagenum_t page_id, enum lock_mode mode, struct trx_t * trx) : table_id(table_id), page_id(page_id), mode(mode), trx(trx){
...
typedef struct table{
	page * header;
	int table_id;
	unordered_map<pagenum_t, lock_t> hash_page;
}table;
...
int begin_tx();
int end_tx(int tid);
buffer * find_leaf(int table_id, int64_t key, int tid);
